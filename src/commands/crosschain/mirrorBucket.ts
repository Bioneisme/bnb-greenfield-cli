...
import { MsgMirrorBucket } from "@bnb-chain/greenfield-cosmos-types/greenfield/storage/tx";
import { config } from "../../utils/config";
import { getPrivateKey } from "../../helpers/password";

export async function mirrorBucket(id: string) {
  try {
    ...
    const msg: MsgMirrorBucket = {
      operator: publicKey,
      id,
    };
    const res = await GreenfieldClient.client.crosschain.mirrorBucket(msg);
    ...
    console.error(`mirror bucket failed: ${e}`);
  }
}
